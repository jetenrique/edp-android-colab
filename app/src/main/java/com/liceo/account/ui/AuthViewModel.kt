...
class AuthViewModel(private val repository: UserRepository = UserRepository()) : ViewModel() {
    ...
    fun login(email: String, password: String) {
        if (email.isBlank() || password.isBlank()) {
            uiState = AuthUiState.Error("Please enter your email and password."); return
        }
    ...
    fun register(fullName: String, email: String, password: String, birthdate: String) {
        if (fullName.isBlank() || email.isBlank() || password.isBlank() || birthdate.isBlank()) {
            uiState = AuthUiState.Error("Please fill in all four fields."); return
        }
        if (!email.contains("@")) {
            uiState = AuthUiState.Error("Please enter a valid email."); return
        }
        if (password.length < 6) {
            uiState = AuthUiState.Error("Password must be at least 6 characters."); return
        }
        if (!datePattern.matches(birthdate.trim())) {
            uiState = AuthUiState.Error("Birthdate must look like 2004-05-17."); return
        }
        uiState = AuthUiState.Loading
        viewModelScope.launch {
