...
@Composable
fun RegisterScreen(state: AuthUiState, onCreate: (String, String, String, String) -> Unit, onGoToLogin: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().systemBarsPadding().imePadding().verticalScroll(rememberScrollState()).padding(24.dp),
        ...
        if (state is AuthUiState.AccountCreated) Text("Account created for ${state.name}. You can now log in.", color = Color(0xFF2E7D32))
        Button({ onCreate(fullName, email, password, birthdate) }, enabled = !isLoading, modifier = Modifier.fillMaxWidth()) {
            if (isLoading) CircularProgressIndicator(Modifier.size(20.dp), strokeWidth = 2.dp) else Text("Create account")
        }
        TextButton(onGoToLogin) { Text("Already have an account? Log in") }
    }
}
